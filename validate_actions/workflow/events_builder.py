...
class BaseEventsBuilder(EventsBuilder):
        ...
        match events_in:
            ...
            case dict():
                for event_in, event_in_value in events_in.items():
                    event_out = self.__build_event_from_dict(
    ...
    def __build_event_from_dict(
        ...
        match event_in.string:
            case 'pull_request' | 'pull_request_target':
                event_out = self.__build_PathsBranchesFilterEvent(
                    event_in, event_in_value
                )
            case 'push':
                event_out = (
                    self.__build_TagsPathsBranchesFilterEvent(
                        event_in, event_in_value
                    )
                )
            case 'schedule':
                event_out = self.__build_ScheduleEvent(
                    event_in, event_in_value
                )
            case 'workflow_call':
                event_out = self.__build_WorkflowCallEvent(
                    event_in, event_in_value
                )
            case 'workflow_run':
                event_out = self.__build_WorkflowRunEvent(
                    event_in, event_in_value
                )
            case 'workflow_dispatch':
                event_out = self.__build_WorkflowDispatchEvent(
                    event_in, event_in_value
                )
            case s if s in self.ALL_EVENTS:
                event_out = self.__build_Event(
                    event_in, event_in_value
                )
            case _:
                self.problems.append(LintProblem(
    ...
    def __build_BranchesFilterEvent(
        self,
        event_type: ast.String,
        filters: Dict[ast.String, Any]
        ...
        return ast.BranchesFilterEvent(
            ...
            types_=types_,
            branches_=branches_,
            branches_ignore_=branches_ignore_
        )
    ...
    def __build_PathsBranchesFilterEvent(
        ...
        parent_filters, addl_filters = self.__split_props(
            filters,
            parent_filter_options
        )

        branches_filter_event = self.__build_BranchesFilterEvent(
            event_type,
            parent_filters
        )
    ...
    def __build_WorkflowCallEvent(
        self,
        event_name: ast.String,
        event_value: Dict[ast.String, Any]
    ):
    ...
    def __build_WorkflowCallEventInputs(
        self,
        inputs_in: Dict[ast.String, Dict[ast.String, Union[ast.String, bool]]]
        ...
        for input_name, input_props in inputs_in.items():
            input = self.__build_WorkflowCallEventInput(
                input_name,
                input_props
            )
    ...
    def __build_WorkflowEventInput(
        ...
        types = {
            'description': (
                ast.String,
                "workflow event input 'description' must be a string"
            ),
            'default': (
                ast.String,
                "workflow event input 'property' default must be a string"
            ),
            'required': (
                bool,
                "workflow event input 'required' must be a boolean"
            ),
        }
    ...
    def __build_WorkflowCallEventInput(
        ...
        base_input_props, call_input_props = self.__split_props(
            input_props,
            base_input_prop_options
        )

        input_base_values = self.__build_WorkflowEventInput(
            base_input_props
        )
        ...
        types = {
            'type': (
                ast.String,
                "Incorrect workflow_call input type"
            )
        }
        ...
        return ast.WorkflowCallEventInput(
            id=input_name,
            description_=cast(
                Optional[ast.String], input_base_values['description']
            ),
            default_=cast(
                Optional[ast.String], input_base_values['default']
            ),
            required_=cast(
                bool, input_base_values['required']
            ),
            type_=type_
        )
    ...
    def __build_WorkflowCallEventOutputs(
        ...
        for output_name, output_props in outputs_in.items():
            output = self.__build_WorkflowCallEventOutput(
                output_name,
                output_props
            )
    ...
    def __build_WorkflowCallEventOutput(
        self,
        output_name: ast.String,
        output_props: Dict[ast.String, ast.String]
        ...
        return ast.WorkflowCallEventOutput(
            id=id,
            value_=value_,
    ...
    def __buildWorkflowCallEventSecrets(
        ...
        secrets_in: Dict[
            ...
            Union[
                ast.String,
                Dict[ast.String, Union[ast.String, bool]]
            ]
        ]
            ...
            if isinstance(secret_value, ast.String):
                ...
                for key, value in secret_value.items():
                    if (
                        ...
                        and isinstance(value, ast.String)
                    ):
                        description_ = value
    ...
    def __build_WorkflowRunEvent(
        ...
        # Leverage parent FilterEvent
        parent_filter_options = ['types', 'branches', 'branches-ignore']
        parent_filters, addl_value = self.__split_props(
            event_value,
            parent_filter_options
        )
        branches_filter_event = self.__build_BranchesFilterEvent(
    ...
    def __build_WorkflowDispatchEvent(
        ...
        return ast.WorkflowDispatchEvent(
            id=event_type,
            inputs_=inputs_
        )
    ...
    def __build_WorkflowDispatchEventInputs(
        ...
        for input_name, input_props in inputs_in.items():
            input = self.__build_WorkflowDispatchEventInput(
                input_name,
                input_props
            )
            # ignores faulty input
    ...
    def __build_WorkflowDispatchEventInput(
        ...
        types = {
            'type': (
                ast.String,
                "Incorrect workflow_dispatch input type"
            ),
            'options': (
                list,
                "workflow_dispatch event input options must be a list"
            )
        }
    ...
    def __split_props(
        self,
        props: Dict[ast.String, Any],
        parent_prop_options: List[str]
        ...
        parent_props = {
            ...
            if key.string in parent_prop_options
        }

        # Extract child/remaining filters
        child_props = {
            key: value for key, value in props.items()
            if key.string not in parent_prop_options
        }
