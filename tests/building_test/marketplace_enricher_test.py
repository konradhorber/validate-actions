...
"""
    ...
    unknown_action_problems = [p for p in problems.problems if "unknown" in p.desc.lower() and p.rule == "marketplace-enricher"]
    assert len(unknown_action_problems) >= 1
    assert unknown_action_problems[0].level == ProblemLevel.WAR


def test_known_action_gets_metadata():
...
def test_marketplace_enricher_direct_usage():
...
def test_marketplace_enricher_handles_missing_action():
